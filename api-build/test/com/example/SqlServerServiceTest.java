...
public class SqlServerServiceTest {
    ...
    public void prePopulate() {
        includedTables.add(new TableRef(TEST_SCHEME, "test"));
...
//        Mockito.doNothing().when(mockSqlServerState).updateLsn(any(), any());
        ...
        doAnswer(new Answer<SqlServerTableInfo>() {
            ...
            public SqlServerTableInfo answer(InvocationOnMock invocationOnMock) throws Throwable {
                ...
                SqlServerTableInfo sqlServerTableInfo = new SqlServerTableInfo(((TableRef) invocationOnMock.getArgument(0)), null, -1, -1, null, -1, new HashSet<>(), null, tableWithSchema, false, null, null);
...
//                System.out.println("mockSqlServerImporter :: importFinished : - " + invocationOnMock.getArgument(0));
        ...
        SqlServerIncrementalUpdater mockIncrementalUpdater = Mockito.mock(SqlServerIncrementalUpdater.class, withSettings().useConstructor(mockSqlServerSource, mockSqlServerState, mockInformer, mockOutput));
        ...
        doAnswer(new Answer<Optional>() {
            ...
            public Optional answer(InvocationOnMock invocationOnMock) throws Throwable {
                ...
                TableRef sourceTable = invocationOnMock.getArgument(1);
                int dummySleep = (Integer.parseInt(sourceTable.name.split("_")[2]) * 100);
                Thread.sleep(dummySleep);

                operations.add(new Operation(sourceTable.name, UPDATER, LocalDateTime.now()));

                return Optional.empty();
            }
        }).when(mockIncrementalUpdater).cdcCatchUp(any(), any(), any(), any(), any());
        ...
        doNothing().when(mockSqlServer).prepareSync(any(), any(), any(), any(), any(), any(), any());

        try {
            mockSqlServer.update(null, null, null, new Output<>(), null, null);
