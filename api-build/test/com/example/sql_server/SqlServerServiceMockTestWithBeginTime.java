 ...
 */
public class SqlServerServiceMockTestWithBeginTime {
...
//        includedTables.add(new TableRef(TEST_SCHEME, "tbl_test_4"));
        ...
        Runnable randomUpdater = new Runnable() {
            ...
            public void run() {
                for (int i = 0; i < 20; i++) {
                    ...
                    MockedDBUtil.updateTable("tbl_test_1", "r");
                    MockedDBUtil.updateTable("tbl_test_2", "r");
                    MockedDBUtil.updateTable("tbl_test_3", "r");
                    try {
                        Thread.sleep(1000);
    ...
    private String getTableFromQuery(String query) {
        if (query.contains("fn_cdc_get_all_changes_dbo_tbl")) {
            return query.split("fn_cdc_get_all_changes_dbo_")[1].substring(0, 10);
        }
    ...
    @Test
    public void testOutOfOrderIssueWithBeginTime() throws Exception {
        //
        System.out.println("#################### testOutOfOrderIssue with Begin Time ##################");
...
//                System.out.println("mockSqlServerImporter :: importFinished : - " + invocationOnMock.getArgument(0));
        ...
        Mockito.doReturn(Boolean.TRUE).when(mockIncrementalUpdater).isLsnValid(any(), any(), any());
...
//        when(mockIncrementalUpdater.getSyncStrategy(anyBoolean(), any(), any(), any(), any())).thenReturn(mockDataSynchronizationStrategy);
        ...
        doAnswer(new Answer<Optional>() {
            ...
            public Optional answer(InvocationOnMock invocationOnMock) throws Throwable {
                ...
                TableRef sourceTable = ((TableDefinition)invocationOnMock.getArgument(0)).getTableRef();
                int dummySleep = (Integer.parseInt(sourceTable.name.split("_")[2]) * 100);
                Thread.sleep(dummySleep);

//                MockedDBUtil.updateTable(sourceTable.name, "u");
                operations.add(new Operation(sourceTable.name, UPDATER, (invocationOnMock.getArgument(2))));
        ...
        doAnswer(new Answer<Optional>() {
            ...
            public Optional answer(InvocationOnMock invocationOnMock) throws Throwable {
                ...
                MockedDBUtil.updateTable(sourceTable.name, "i");

                operations.add(new Operation(sourceTable.name, IMPORTER, mockSqlServerState.getImportBeginTime(sourceTable)));
...
//        when(mockConnection.prepareStatement(any())).thenReturn(mockPreparedStatement);
        doAnswer(new Answer<PreparedStatement>() {
            @Override
            public PreparedStatement answer(InvocationOnMock invocationOnMock) throws Throwable {
                MockedDBUtil.addQuery(invocationOnMock.getArgument(0));
        ...
        doAnswer(new Answer<Boolean>() {
            @Override
            public Boolean answer(InvocationOnMock invocationOnMock) throws Throwable {
                String query = MockedDBUtil.getQuery();
                if (query == null) {
                    return MockedDBUtil.isHistoryAvl();
                }
                String table = getTableFromQuery(query);
                if (table != null) {
                    MockedDBUtil.addQuery(query);
                    return MockedDBUtil.isHistoryAvl(table);
                }
...
//        when(mockResultSet.getBytes(any())).thenReturn(bigIntToByteArray(index++));
        doAnswer(new Answer<byte[]>() {
            @Override
            public byte[] answer(InvocationOnMock invocationOnMock) throws Throwable {
                return bigIntToByteArray(index++);
            }
        }).when(mockResultSet).getBytes(any());
        doAnswer(new Answer<Timestamp>() {
            @Override
            public Timestamp answer(InvocationOnMock invocationOnMock) throws Throwable {
                ...
                String table = getTableFromQuery(query);
                if (table != null) {
                    if (MockedDBUtil.isHistoryAvl(table)) {
                        MockedDBUtil.addQuery(query);
                    }
                    return MockedDBUtil.getTimestamp(table);
                }

                return null;
            }
        }).when(mockResultSet).getTimestamp(any());
        when(mockResultSet.getInt(any())).thenReturn(4);
        ...
        Assert.assertTrue("Update happened before import for tbl_test_1", importShouldHappenAtOrBeforeUpdate("tbl_test_1"));
        Assert.assertTrue("Update happened before import for tbl_test_2", importShouldHappenAtOrBeforeUpdate("tbl_test_2"));
        Assert.assertTrue("Update happened before import for tbl_test_3", importShouldHappenAtOrBeforeUpdate("tbl_test_3"));
        Assert.assertTrue("Update happened before import for tbl_test_4", importShouldHappenAtOrBeforeUpdate("tbl_test_4"));

        System.out.println("----------------------------- Process Completed --------------------------------");
    }

    public boolean importShouldHappenAtOrBeforeUpdate(String tableName) {
        // sort by time
        List<Operation> operationsPerTable = operations.stream().filter(operation -> operation.getTableName().equals(tableName))
                .sorted(Comparator.comparing(Operation::getSource).thenComparing(Operation::getTime))
                .collect(Collectors.toList());

        // first record is by importer, 2nd (and all the others are by updater and sorted by time)
        if (operationsPerTable.size() > 0) {
            return !(operationsPerTable.get(0).getTime().isAfter(operationsPerTable.get(1).getTime()));
        }
        return true;
    }
}