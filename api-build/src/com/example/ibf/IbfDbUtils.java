...
import com.example.ibf.db_compare.IbfCompareIntermediateRow;
...
public class IbfDbUtils {
    ...
    public static int computeKeyLength(long typeParameter) {
        if (typeParameter <= SINGLE_KEY_SIZE) {
            return IbfTableEncoder.DEFAULT_KEY_LENGTH;
        }
        int keyLength = (int) (typeParameter / SINGLE_KEY_SIZE);
        if (typeParameter % SINGLE_KEY_SIZE > 0) keyLength += 1;
        return keyLength;
    }
    ...
    private static String decodeStringPk(long[] keySum, int offset, int keyLength) {
        ...
        for (int i = 0; i < keyLength; i++) {
            ...
            try {
                ...
                LOG.severe("Failing to decode String primary key: " + e.getMessage());
